...
export const generateMetadata = async ({ params }) => {
  const { title, content, tags } = blogs.find(
...
const page = ({ params }) => {
  const { title, content, tags, date, comments } = blogs.find(
    (blog) => blog.slug === params.slug
  );
  return (
    <div className="px-16">
      <div className="border p-5 space-y-5">
        ...
        <div className="flex justify-between items-center">
          ...
          <p>
            Comments:
            <span className="bg-emerald-400 text-black px-2 py-1 rounded-full ml-2">
              {comments.length}
            </span>
          </p>
        </div>
      ...
      <div className="flex gap-10 justify-between items-center mt-10">
        {comments.map((comment, idx) => (
          <div
            key={idx}
            className="flex flex-col items-center gap-2 border rounded-lg px-5 py-8"
          >
            <h1 className="underline">{comment.author}</h1>
